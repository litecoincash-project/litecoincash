...
def main():
    ...
    tmpdir = "%s/neon_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and neond must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "neond"]) is not None:
    ...
    if "NEOND" not in os.environ:
        os.environ["NEOND"] = build_dir + '/src/neond' + exeext
        os.environ["NEONCLI"] = build_dir + '/src/neon-cli' + exeext
