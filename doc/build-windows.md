...
Below are some notes on how to build Neon Core for Windows.

The options known to work for building Neon Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Neon Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Neon Core applications.
...
Note that for WSL the Neon Core source path MUST be somewhere in the default mount file system, for
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/neon-project/neon.git
    sudo chmod -R a+rw neon
...
causes two of the neon executables to crash shortly after start up. The bug is related to the
